    ...
    for(i=0; i<products.length; i++){
        ...
        productele.innerHTML = `  <h3>${product.title}</h3>
                                    ...
                                    <div class="prod-details">
                                        <span>₹<span class="price">${product.price.toLocaleString()}</span></span>
...
async function getmerchproducts(page) {
    console.log(`Getting MERCH products for page ${page}`);
    const response = await axios.get(`http://${IP}:4000/merch?page=${page}`);
    showmerchproducts(response.data.products);
    showmerchpages(response.data);
}
